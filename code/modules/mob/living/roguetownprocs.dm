...
/mob/proc/do_parry(obj/item/W, parrydrain as num, mob/living/user)
	if(ishuman(src))
		...
		if(H.rogfat_add(parrydrain))
			...
			if(src.client)
				GLOB.blackmoor_round_stats[STATS_PARRIES]++
			if(istype(rmb_intent, /datum/rmb_intent/riposte))
...
/mob/living/carbon/human/proc/measured_statcheck(mob/living/carbon/human/HT)
	...
	var/max_target = min(max(HT.STASTR, HT.STACON, HT.STAINT, HT.STAPER, HT.STASPD), 14)
	var/min_target = min(HT.STASTR, HT.STACON, HT.STAINT, HT.STAPER, HT.STASPD)
	var/max_user = min(max(STASTR, STACON, STAINT, STAPER, STASPD), 14)
	var/min_user = min(STASTR, STACON, STAINT, STAPER, STASPD)
